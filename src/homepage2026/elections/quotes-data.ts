export type Quote = {
  author?: string
  context?: string
  quote: string
}

export const QUOTES: Quote[] = [
  {
    quote:
      "This went really well... and thank you for the kind words! Our whole data team was over the moon. We should do a post mortem to compare notes but I think this time next year we'll have the best voting system including registration in the country.",
  },
  { quote: "Amazing! Couldn't have been better." },
  { quote: 'Easy! Keeps getting better and better.' },
  {
    author: 'Phil Boileau',
    context: 'Forward Party Organizer · December 2025 vote',
    quote:
      'SIV is on the front lines of a truly important technology with incredible potential. The broader concept of online voting has the potential to shift paradigms around how and what it means to vote and, in doing so, strengthen global democracy. But any approach must begin with an absolute commitment to security, and this is where SIV most clearly sets itself apart.\n\nOur election had to be fast, adaptable, utterly secure, able to withstand academic and political scrutiny, and delivered at a fraction of the cost typically required to run an election. SIV delivered on every front.',
  },
  { context: 'Private election', quote: 'Very cool! Wow, SIV JUST WORKS!' },
  { context: 'Private election', quote: 'This vote was a total success, thanks in very large part to you!' },
  { context: 'Private election', quote: 'Thank you both SO MUCH! You sure do build beautiful software!' },
  {
    context: 'Private election',
    quote:
      "Everything has gone PERFECTLY! I'm so impressed! I posted this to the team last week. Thank you both for incredible technology. You build beautiful software!",
  },
  {
    context: 'Private election',
    quote:
      "Have to callout how intuitive and clean the SIV admin layout is. What a huge step forward it is to have transitioned from Helios to SIV. I can't say enough about SIV as a product, the support, the alignment with our community's ethos, etc. This was a win for everyone across the board.",
  },
  {
    context: 'Private election',
    quote:
      "Hello! Thanks for thinking of us! I have been enjoying the ease of admin-ing this poll. It's amazing how calm I feel compared to the old Helios days! Thank you for building SIV! 🙌",
  },
]
