import { execFileSync } from 'child_process'
import { statSync } from 'fs'
import { GetStaticProps } from 'next'
import { join } from 'path'

const FILE = 'src/bug-bounty/BugBountyPage.tsx'
const PROGRAM_START = '2026-08-14'

export function formatLastUpdated(isoDate: string) {
  const [y, m, d] = isoDate.slice(0, 10).split('-').map(Number)
  return new Date(Date.UTC(y, m - 1, d)).toLocaleDateString('en-US', {
    weekday: 'long',
    year: 'numeric',
    month: 'long',
    day: 'numeric',
    timeZone: 'UTC',
  })
}

function localIsoDate(d: Date) {
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`
}

export function lastUpdatedIso(file = FILE) {
  const committed = execFileSync('git', ['log', '-1', '--format=%cI', '--', file], { encoding: 'utf8' }).trim()
  const dirty = execFileSync('git', ['status', '--porcelain', '--', file], { encoding: 'utf8' }).trim()
  if (!dirty) return committed
  return localIsoDate(statSync(join(process.cwd(), file)).mtime)
}

export function formatActiveFor(nowIso = localIsoDate(new Date()), start = PROGRAM_START) {
  const [sy, sm, sd] = start.split('-').map(Number)
  const [ny, nm, nd] = nowIso.slice(0, 10).split('-').map(Number)
  let years = ny - sy
  let months = nm - sm
  if (nd < sd) months--
  if (months < 0) {
    years--
    months += 12
  }
  if (years >= 1) {
    if (!months) return years === 1 ? '1 year' : `${years} years`
    return `${years}.${months} years`
  }
  if (months >= 1) return months === 1 ? '1 month' : `${months} months`
  const days = Math.max(1, Math.floor((Date.UTC(ny, nm - 1, nd) - Date.UTC(sy, sm - 1, sd)) / 86_400_000))
  return days === 1 ? '1 day' : `${days} days`
}

export const getStaticProps: GetStaticProps<{ activeFor: string; lastUpdated: string }> = async () => ({
  props: { activeFor: formatActiveFor(), lastUpdated: formatLastUpdated(lastUpdatedIso()) },
  revalidate: 86_400,
})
